...
(* Doesn't work, doesn't check for physical equality! *)

(* let has_cycle (lst : 'a mlist) : bool =
  let rec helper (ls: 'a mlist) (visited : 'a list) : bool =
    match ls with
    | Nil -> false
    | Cons(hd, {contents = tl}) ->
      if List.memq hd visited then false
      else helper tl (hd :: visited)
  in helper lst [];; *)
